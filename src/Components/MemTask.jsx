...
class MemTask extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      stairDirEasy: ["up", "up"], // valid initial direction for staircase
      ...
      stairDirHard: ["up", "up"], // valid initial direction for staircase
  ...
  renderStim() {
    ...
    // Merge dotStar update into the same setState call to avoid a second render
    this.setState({
      instructScreen: false,
      taskScreen: true,
  ...
  // renderTaskSave
  //
  // Saves per-trial data. Includes:
  //   - responseMatrix (combined log)
  //   - responseMatrixEasy / responseMatrixHard (block-specific logs)
  //   - stairCountEasy / stairCountHard (now saved, previously missing)
  // ─────────────────────────────────────────────────────────────────────────
  renderTaskSave() {
    ...
    // Both branches (end of block, end of task) call restBlock — collapsed into one check
