...
class Questionnaires extends React.Component {
  constructor(props) {
    ...
    let quizLabel = ["SDS", "STAIY2"]; //add the quiz labels here e.g. "RSE"
    let allQuizText = [sds, staiy2]; //add the quiz variable in here
  ...
  handleGlobalMouseMove(event) {
    if (this.state.questScreen && !this.ticking) {
      window.requestAnimationFrame(() => {
        ...
        if (this.state.section === "psych") sectionTag = "p";
